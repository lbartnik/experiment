...
(function () {
  var Data, Position, UI, Widget, Widget2;
  ...
  UI = function UI(selection) {
    var canvas, createGraphics, linksG, nodeR, nodesG, outer, ui;
    outer = null;
    canvas = null;
    linksG = null;
    nodesG = null;
    ...
    createGraphics = function createGraphics(data) {
      ...
      enter = node.enter().append("svg").attr("class", function (d) {
        ...
        return d.id;
      }).attr("viewBox", "0 0 " + 2 * nodeR + " " + 2 * nodeR).attr("width", 2 * nodeR).attr("height", 2 * nodeR);
      enter.each(function (d) {
        var element;
        element = d3.select(this);
        if (d.type === 'object') {
          element.append("rect").attr("width", 2 * nodeR).attr("height", 2 * nodeR).attr("rx", nodeR / 4).attr("ry", nodeR / 4);
          ...
          return element.append("rect").attr("class", "face").attr("width", 2 * nodeR).attr("height", 2 * nodeR).attr("rx", nodeR / 4).attr("ry", nodeR / 4);
        } else {
          if (d.contents) {
            return element.append("image").attr("width", 2 * nodeR).attr("height", 2 * nodeR).attr("xlink:href", $("#plot" + d.id).attr("href"));
          } else {
            return element.append("rect").attr('width', 2 * nodeR).attr('height', 2 * nodeR).style("fill", "grey");
          }
        }
      });
      node.exit().remove();
  ...
  // --- Data -------------------------------------------------------------
  Position = function Position(width, height) {
    ...
    return position.calculate = function (data) {
      var min_x, min_y, nodesMap, s, t;
      s = stratified(data);
      t = treed(s);
      min_x = t.descendants().map(function (n) {
        return n.x;
      }).reduce(function (a, b) {
        return Math.min(a, b);
      });
      min_y = t.descendants().map(function (n) {
        return n.y;
      }).reduce(function (a, b) {
        return Math.min(a, b);
      });
  ...
  Widget2 = function Widget2(selection) {
    var addPlot, createVisuals, data, enableEvents, filterData, hidePlot, hideVariable, lenses, lenses_r, linksG, mapNodes, moveLenses, nodesG, placeVisuals, refreshVisuals, setupData, showPlot, showVariable, template, thumbnail, timeout, toClipboard, updatePositions, vis, widget, zoomFrame, zoomed;
    lenses_r = 50;
    timeout = 150;
    thumbnail = 25;
    zoomed = 250;
